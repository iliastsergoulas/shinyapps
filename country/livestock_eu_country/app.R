...
server <- function(input, output) {
    ...
    output$view <- renderGvis({ # Creating chart
        gvisColumnChart(data_country(), options=list(colors="['#336600']", title="Ζωικό κεφάλαιο στις χώρες της Ε.Ε.", 
                                        titleTextStyle="{color:'#336600',fontSize:14}", vAxis="{title:'Μέγεθος ζωικού κεφαλαίου (χιλ. κεφαλές)'}", hAxis="{title:'Έτος'}",
    ...
    output$timeline<-renderPlot({ # Creating timeline for top 5 countries
        ggplot(mydata_top_five(), aes(x = year, y = number, group = country, colour = country)) + 
            ...
            xlab("Έτος") + ylab("Μέγεθος ζωικού κεφαλαίου (χιλ. κεφαλές)") + ggtitle("5 χώρες με το μεγαλύτερο ζωικό κεφάλαιο") + 
