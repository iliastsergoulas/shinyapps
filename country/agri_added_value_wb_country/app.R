...
mydata<-WDI(country = "all", indicator = "NV.AGR.TOTL.CD", extra = FALSE, cache = NULL) # Downloading raw data from World Bank
mydata$year<-as.character(mydata$year)
names(mydata)[names(mydata)=="NV.AGR.TOTL.CD"] <- "total_agri_exports"
...
server <- function(input, output) {
    data_country <- reactive({ # Add reactive data information
        ...
        colnames(data_country)<-c("Έτος", "Προστιθέμενη αξία γεωργίας (δολάρια)")
        data_country
    })
    data_year <- reactive({ # Add reactive data information
        ...
        colnames(data_year)<-c("Χώρα", "Προστιθέμενη αξία γεωργίας (δολάρια)")
        data_year
    })
    ...
    output$view <- renderGvis({ # Creating chart
        gvisColumnChart(data_country(), options=list(colors="['#336600']", vAxis="{title:'Προστιθέμενη αξία γεωργίας (δολάρια)'}", 
                        hAxis="{title:'Έτος'}",backgroundColor="#d9ffb3", width=700, height=500, legend='none'))
    })
    output$map <- renderGvis({
        gvisGeoChart(data_year(), "Χώρα", "Προστιθέμενη αξία γεωργίας (δολάρια)", options=list(displayMode="regions", datamode='regions',width=700, height=500))
    })
    output$table <- renderDataTable({ # Creating data table
        colnames(mydata)<-c("Κωδικός", "Χώρα", "Προστιθέμενη αξία γεωργίας", "Έτος")
        mydata[c("Χώρα", "Προστιθέμενη αξία γεωργίας", "Έτος")]
    })
    output$summary <- renderDataTable({ # Creating data table
        ...
        colnames(mysummary) <- c("Χώρα", "Ελάχιστη προστιθέμενη αξία εξαγωγών", "Μέγιστη προστιθέμενη αξία εξαγωγών", "Μέση προστιθέμενη αξία εξαγωγών")
        mysummary
    })
    output$timeline<-renderPlot({
        ggplot(mydata_top_five(), aes(x = year, y = total_agri_exports, group = country, colour = country)) + 
            ...
            xlab("Έτος") + ylab("Προστιθέμενη αξία εξαγωγών") + ggtitle("5 κορυφαίες χώρες εξαγωγείς αγροτικών προϊόντων - Προστιθέμενη αξία") + 
