...
header <- dashboardHeader(title = "Ποσοστό αγροτικού πληθυσμού", titleWidth=500) # Header of dashboard
...
frow3 <- fluidRow(# Creating row of diagram and summary
    box(
        title = "5 χώρες με μεγαλύτερο ποσοστό αγροτικού πληθυσμού",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
...
server <- function(input, output) {
    data_country <- reactive({ # Adding reactive data information
        ...
        colnames(data_country)<-c("Έτος", "Ποσοστό αγροτικού πληθυσμού")
        data_country
    })
    data_year <- reactive({ # Addding reactive data information
        ...
        colnames(data_year)<-c("Χώρα", "Ποσοστό αγροτικού πληθυσμού")
        data_year
    })
    ...
    output$map <- renderGvis({ # Creating map
        gvisGeoChart(data_year(), "Χώρα", "Ποσοστό αγροτικού πληθυσμού", 
                     options=list(displayMode="regions", datamode='regions',width=550, height=500))
    })
    output$table <- renderDataTable({ # Creating data table
        colnames(mydata)<-c("Κωδικός", "Χώρα", "Ποσοστό αγροτικού πληθυσμού", "Έτος")
        mydata[c("Χώρα", "Ποσοστό αγροτικού πληθυσμού", "Έτος")]
    })
    output$summary <- renderDataTable({ # Creating summary by country
        ...
        colnames(mysummary) <- c("Χώρα", "Ελάχιστο ποσοστό αγροτικού πληθυσμού", "Μέγιστο ποσοστό αγροτικού πληθυσμού", "Μέσο ποσοστό αγροτικού πληθυσμού")
        mysummary
    ...
    output$agri_population_percentage <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Μέσο ποσοστό αγροτικού πληθυσμού παγκοσμίως",
            icon = icon("user"),
            color = "olive")
    })
    output$topcountry <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Χώρα με μεγαλύτερο ποσοστό αγροτικού πληθυσμού",
            icon = icon("globe"),
            color = "olive")
    })
