# Data: Economically active population in agriculture (number)
...
mydata<-WDI(country = "all", indicator = "EN.AGR.EMPL", extra = FALSE, cache = NULL) # Downloading raw data from World Bank
...
meanvalue<-mean((aggregate(mydata_filtered$agri_econ_population, by=list(year=mydata_filtered$year), FUN=mean)$x)) # Mean value
topc<-mydata_filtered[which.max(mydata_filtered$agri_econ_population),] # Top country
header <- dashboardHeader(title = "Oικονομικά ενεργός πληθυσμός γεωργίας", titleWidth=500) # Header of dashboard
sidebar <- dashboardSidebar(disable = TRUE)# Disabling sidebar of dashboard
frow1 <- fluidRow( # Creating row of valueboxes
    valueBoxOutput("agri_econ_population", width=6),
...
frow3 <- fluidRow(# Creating row of diagram and summary
    box(
        title = "5 χώρες με μεγαλύτερo οικονομικά ενεργό πληθυσμό γεωργίας",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
...
server <- function(input, output) {
    data_country <- reactive({ # Adding reactive data information
        data_country<-mydata[mydata$country==input$country, c("year", "agri_econ_population")]
        data_country<-aggregate(data_country$agri_econ_population, by=list(Year=data_country$year), FUN=sum)
        colnames(data_country)<-c("Έτος", "Πληθυσμός")
        data_country
    })
    data_year <- reactive({ # Addding reactive data information
        data_year<-mydata_filtered[mydata_filtered$year==input$year,  c("country", "agri_econ_population")]
        data_year<-aggregate(data_year$agri_econ_population, by=list(Country=data_year$country), FUN=sum)
        colnames(data_year)<-c("Χώρα", "Πληθυσμός")
        data_year
    })
    mydata_top_five<-reactive({ # Subsetting data according to year interval and getting top five countries
        ...
        data_year_temp<-aggregate(mydata_top_five$agri_econ_population, by=list(Country=mydata_top_five$country), FUN=mean)
    ...
    output$map <- renderGvis({ # Creating map
        gvisGeoChart(data_year(), "Χώρα", "Πληθυσμός", options=list(displayMode="regions", datamode='regions',width=700, height=500))
    })
    output$table <- renderDataTable({ # Creating data table
        colnames(mydata)<-c("Κωδικός", "Χώρα", "Πληθυσμός", "Έτος")
    ...
    output$topcountry <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Χώρα με μεγαλύτερο οικονομικά ενεργό πληθυσμό γεωργίας",
            icon = icon("globe"),
            color = "olive")
    })
    output$timeline<-renderPlot({ # Creating timeline for top 5 countries
        ggplot(mydata_top_five(), aes(x = year, y = agri_econ_population, group = country, colour = country)) + 
            geom_line() +
