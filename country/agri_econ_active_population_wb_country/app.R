...
server <- function(input, output) {
    ...
    output$view <- renderGvis({ # Creating chart
        gvisColumnChart(data_country(), options=list(colors="['#336600']", title="Οικονομικά ενεργός πληθυσμός γεωργίας", 
    ...
    output$timeline<-renderPlot({ # Creating timeline for top 5 countries
        ggplot(mydata_top_five(), aes(x = year, y = agri_econ_population, group = country, colour = country)) + 
            ...
            xlab("Έτος") + ylab("Πληθυσμός") + ggtitle("5 χώρες με μεγαλύτερο οικονομικά ενεργό πληθυσμό γεωργίας") + 
