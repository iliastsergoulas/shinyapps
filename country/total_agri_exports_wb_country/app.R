...
server <- function(input, output) {
    ...
    output$summary <- renderDataTable({ # Creating summary by country
        mysummary <- data.frame(
            ...
            aggregate(total_agri_exports~country, mydata_summary(), median))
        mysummary <- mysummary[,c(1,2,4,6,8)]
        colnames(mysummary) <- c("Χώρα", "Ελάχιστη αξία εξαγωγών", "Μέγιστη αξία εξαγωγών", "Μέση αξία εξαγωγών", "Διάμεσος")
        mysummary
    })
