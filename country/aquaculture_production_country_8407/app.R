...
mydata<-get_eurostat("fish_aq_q", time_format = "raw")
mydata$geo<-as.character(mydata$geo)
mydata<-mydata[which(mydata$unit=='TLW' & mydata$aquaenv=='TOTAL'& mydata$fishreg==0 & mydata$species=='F00'), ]
...
server <- function(input, output) {
    ...
    output$map <- renderGvis({
        geomap <- gvisGeoChart(data_year(), "Χώρα", "Παραγωγή", options=list(region="150", displayMode="regions", 
