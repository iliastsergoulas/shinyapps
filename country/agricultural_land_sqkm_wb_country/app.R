...
server <- function(input, output) {
    ...
    mydata_top_five<-reactive({ # Subsetting data according to year interval and getting top five countries
        mydata_filtered<-mydata_filtered[which(mydata_filtered$year>=input$myyear[1] & mydata_filtered$year<=input$myyear[2]),]
    ...
    output$view <- renderGvis({ # Creating chart
        gvisColumnChart(data_country(), options=list(colors="['#336600']", title="Έκταση αγροτικής γης (τ.χλμ.)", 
                        titleTextStyle="{color:'#336600',fontSize:14}", vAxis="{title:'Έκταση αγροτικής γης (τ.χλμ.)'}", 
    ...
    output$summary <- renderDataTable({ # Creating summary by country
        mysummary <- data.frame(
            ...
            aggregate(agri_area_sqkm~country, mydata_summary(), median))
        mysummary <- mysummary[,c(1,2,4,6, 8)]
        colnames(mysummary) <- c("Χώρα", "Ελάχιστη έκταση αγροτικής γης", "Μέγιστη έκταση αγροτικής γης", "Μέση έκταση αγροτικής γης", "Διάμεσος")
        mysummary
    })
    output$timeline<-renderPlot({ # Creating timeline for top 5 countries
        ggplot(mydata_top_five(), aes(x = year, y = agri_area_sqkm, group = country, colour = country)) + 
            ...
            xlab("Έτος") + ylab("Ποσοστό αγροτικής γης") + ggtitle("5 χώρες με μεγαλύτερη έκταση αγροτικής γης") + 
