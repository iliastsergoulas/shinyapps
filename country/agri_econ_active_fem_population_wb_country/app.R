...
header <- dashboardHeader(title = "Oικονομικά ενεργός γυναικείος πληθυσμός γεωργίας", titleWidth=500) # Header of dashboard
...
frow3 <- fluidRow(# Creating row of diagram and summary
    box(
        title = "5 χώρες με μεγαλύτερo οικονομικά ενεργό γυναικείο πληθυσμό γεωργίας",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
...
server <- function(input, output) {
    ...
    output$topcountry <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Χώρα με μεγαλύτερο οικονομικά ενεργό γυναικείο πληθυσμό γεωργίας",
            icon = icon("globe"),
            color = "olive")
    })
    output$summary <- renderDataTable({ # Creating summary by country
        ...
        colnames(mysummary) <- c("Χώρα", "Ελάχιστος οικονομικά ενεργός γυναικείος πληθυσμός γεωργίας", "Μέγιστος οικονομικά ενεργός γυναικείος πληθυσμός γεωργίας", "Μέσος οικονομικά ενεργός γυναικείος πληθυσμός γεωργίας")
        mysummary
