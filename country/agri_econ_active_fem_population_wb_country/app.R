...
mydata<-WDI(country = "all", indicator = "EN.AGR.EMPL.FE", extra = FALSE, cache = NULL) # Downloading raw data from World Bank
...
server <- function(input, output) {
    ...
    output$view <- renderGvis({ # Creating chart
        gvisColumnChart(data_country(), options=list(colors="['#336600']", title="Οικονομικά ενεργός γυναικείος πληθυσμός γεωργίας", 
    ...
    output$summary <- renderDataTable({ # Creating summary by country
        mysummary <- data.frame(
            ...
            aggregate(agri_econ_fem_population~country, mydata_summary(), median))
        mysummary <- mysummary[,c(1,2,4,6,8)]
        colnames(mysummary) <- c("Χώρα", "Ελάχιστος οικονομικά ενεργός γυναικείος πληθυσμός γεωργίας", "Μέγιστος οικονομικά ενεργός γυναικείος πληθυσμός γεωργίας", "Μέσος οικονομικά ενεργός γυναικείος πληθυσμός γεωργίας", "Διάμεσος")
        mysummary
    })
    output$timeline<-renderPlot({ # Creating timeline for top 5 countries
        ggplot(mydata_top_five(), aes(x = year, y = agri_econ_fem_population, group = country, colour = country)) + 
            ...
            xlab("Έτος") + ylab("Πληθυσμός") + ggtitle("5 χώρες με μεγαλύτερο οικονομικά ενεργό γυναικείο πληθυσμό γεωργίας") + 
