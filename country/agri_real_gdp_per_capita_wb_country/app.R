...
header <- dashboardHeader(title = "Ρυθμός μεταβολής κ.κ. πραγματικού ΑΕΠ γεωργίας (%)", titleWidth=500) # Header of dashboard
...
server <- function(input, output) {
    ...
    output$map <- renderGvis({ # Creating map
        gvisGeoChart(data_year(), "Χώρα", "Ρυθμός μεταβολής κατά κεφαλήν ΑΕΠ γεωργίας (%)", 
    ...
    output$topcountry <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Χώρα με μεγαλύτερο ρυθμό μεταβολής κατά κεφαλήν πραγματικού ΑΕΠ γεωργίας (%)",
            icon = icon("globe"),
            color = "olive")
    })
    output$timeline<-renderPlot({ # Creating timeline for top 5 countries
        ggplot(mydata_top_five(), aes(x = year, y = agri_real_gdp_per_capita, group = country, colour = country)) + 
            ...
            xlab("Έτος") + ylab("Ρυθμός μεταβολής κατά κεφαλήν πραγματικού ΑΕΠ γεωργίας (%)") + 
