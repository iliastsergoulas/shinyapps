# Data: Rural population growth (annual %)
# This R script is created as a Shiny application processing raw data downloaded from World Bank through WDI package, 
# and creating plots and maps as WDI(country = "all", indicator = "SP.RUR.TOTL.ZG", extra = FALSE, cache = NULL)
...
mydata <- dbGetQuery(con, "SELECT * from agriculture.agri_population_rate_wb_country") # Get data
dbDisconnect(con)
dbUnloadDriver(drv)
mydata$year<-as.character(mydata$year)
names(mydata)[names(mydata)=="SP.RUR.TOTL.ZG"] <- "agri_population_rate"
mydata<-mydata[which(!is.na(mydata$agri_population_rate)),] # Filtering for NA values
...
meanvalue<-mean((aggregate(mydata_filtered$agri_population_rate, by=list(year=mydata_filtered$year), FUN=mean)$x)) # Mean value
topc<-mydata_filtered[which.max(mydata_filtered$agri_population_rate),] # Top country
header <- dashboardHeader(title = "Ρυθμός μεταβολής αγροτικού πληθυσμού", titleWidth=500) # Header of dashboard
sidebar <- dashboardSidebar(disable = TRUE)# Disabling sidebar of dashboard
frow1 <- fluidRow( # Creating row of valueboxes
    valueBoxOutput("agri_population_rate", width=6),
...
frow3 <- fluidRow(# Creating row of diagram and summary
    box(
        title = "5 χώρες με μεγαλύτερο ρυθμό μεταβολής αγροτικού πληθυσμού",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
...
server <- function(input, output) {
    data_country <- reactive({ # Adding reactive data information
        data_country<-mydata[mydata$country==input$country, c("year", "agri_population_rate")]
        data_country<-aggregate(data_country$agri_population_rate, by=list(Year=data_country$year), FUN=sum)
        colnames(data_country)<-c("Έτος", "Ρυθμός μεταβολής αγροτικού πληθυσμού")
        data_country
    })
    data_year <- reactive({ # Addding reactive data information
        data_year<-mydata_filtered[mydata_filtered$year==input$year,  c("country", "agri_population_rate")]
        data_year<-aggregate(data_year$agri_population_rate, by=list(Country=data_year$country), FUN=sum)
        colnames(data_year)<-c("Χώρα", "Ρυθμός μεταβολής αγροτικού πληθυσμού")
        data_year
    })
    mydata_top_five<-reactive({ # Subsetting data according to year interval and getting top five countries
        ...
        data_year_temp<-aggregate(mydata_top_five$agri_population_rate, by=list(Country=mydata_top_five$country), FUN=mean)
    ...
    output$view <- renderGvis({ # Creating chart
        gvisColumnChart(data_country(), options=list(colors="['#336600']", vAxis="{title:'Ρυθμός'}", 
                        hAxis="{title:'Έτος'}",backgroundColor="#d9ffb3", width=550, height=500, legend='none'))
    })
    output$map <- renderGvis({ # Creating map
        gvisGeoChart(data_year(), "Χώρα", "Ρυθμός μεταβολής αγροτικού πληθυσμού", 
                     options=list(displayMode="regions", datamode='regions',width=550, height=500))
    })
    output$table <- renderDataTable({ # Creating data table
        colnames(mydata)<-c("Κωδικός", "Χώρα", "Ρυθμός μεταβολής αγροτικού πληθυσμού", "Έτος")
        mydata[c("Χώρα", "Ρυθμός μεταβολής αγροτικού πληθυσμού", "Έτος")]
    })
    output$summary <- renderDataTable({ # Creating summary by country
        mysummary <- data.frame(
            aggregate(agri_population_rate~country, mydata_summary(), min),
    ...
    output$agri_population_rate <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Μέσος ρυθμός μεταβολής αγροτικού πληθυσμού παγκοσμίως",
            icon = icon("user"),
            color = "olive")
    })
    output$topcountry <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Χώρα με μεγαλύτερο ρυθμό μεταβολής αγροτικού πληθυσμού",
            icon = icon("globe"),
            color = "olive")
    })
    output$timeline<-renderPlot({ # Creating timeline for top 5 countries
        ggplot(mydata_top_five(), aes(x = year, y = agri_population_rate, group = country, colour = country)) + 
            geom_line() +
