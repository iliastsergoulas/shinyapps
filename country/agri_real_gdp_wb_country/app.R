...
server <- function(input, output) {
    ...
    output$view <- renderGvis({ # Creating chart
        gvisColumnChart(data_country(), options=list(colors="['#336600']", title="Ρυθμός μεταβολής ΑΕΠ γεωργίας (%)", 
    ...
    output$summary <- renderDataTable({ # Creating summary by country
        mysummary <- data.frame(
            ...
            aggregate(agri_real_gdp~country, mydata_summary(), median))
        mysummary <- mysummary[,c(1,2,4,6, 8)]
        colnames(mysummary) <- c("Χώρα", "Ελάχιστος ρυθμός μεταβολής ΑΕΠ γεωργίας (%)", "Μέγιστος ρυθμός μεταβολής ΑΕΠ γεωργίας (%)", "Μέσος ρυθμός μεταβολής ΑΕΠ γεωργίας (%)", "Διάμεσος")
        mysummary
    })
    output$timeline<-renderPlot({ # Creating timeline for top 5 countries
        ggplot(mydata_top_five(), aes(x = year, y = agri_real_gdp, group = country, colour = country)) + 
            ...
            xlab("Έτος") + ylab("Ρυθμός μεταβολής ΑΕΠ γεωργίας (%)") + ggtitle("5 χώρες με μεγαλύτερο ρυθμό μεταβολής ΑΕΠ γεωργίας (%)") + 
