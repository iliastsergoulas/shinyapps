...
header <- dashboardHeader(title = "Ρυθμός μεταβολής πραγματικού ΑΕΠ γεωργίας (%)", titleWidth=500) # Header of dashboard
...
frow3 <- fluidRow(# Creating row of diagram and summary
    box(
        title = "5 χώρες με μεγαλύτερο ρυθμό μεταβολής ΑΕΠ γεωργίας (%)",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
...
server <- function(input, output) {
    data_country <- reactive({ # Adding reactive data information
        ...
        colnames(data_country)<-c("Έτος", "Ρυθμός μεταβολής ΑΕΠ γεωργίας (%)")
        data_country
    })
    data_year <- reactive({ # Adding reactive data information
        ...
        colnames(data_year)<-c("Χώρα", "Ρυθμός μεταβολής ΑΕΠ γεωργίας (%)")
        data_year
    })
    ...
    output$view <- renderGvis({ # Creating chart
        gvisColumnChart(data_country(), options=list(colors="['#336600']", vAxis="{title:'Αξία (δολάρια σε τιμές 2000)'}", 
                        hAxis="{title:'Έτος'}",backgroundColor="#d9ffb3", width=700, height=500, legend='none'))
    })
    output$map <- renderGvis({ # Creating map
        gvisGeoChart(data_year(), "Χώρα", "Ρυθμός μεταβολής ΑΕΠ γεωργίας (%)", options=list(displayMode="regions", datamode='regions',width=700, height=500))
    })
    output$table <- renderDataTable({ # Creating data table
        colnames(mydata)<-c("Κωδικός", "Χώρα", "Ρυθμός μεταβολής ΑΕΠ γεωργίας (%)", "Έτος")
        mydata[c("Χώρα", "Ρυθμός μεταβολής ΑΕΠ γεωργίας (%)", "Έτος")]
    })
    output$summary <- renderDataTable({ # Creating summary by country
        ...
        colnames(mysummary) <- c("Χώρα", "Ελάχιστος ρυθμός μεταβολής ΑΕΠ γεωργίας (%)", "Μέγιστος ρυθμός μεταβολής ΑΕΠ γεωργίας (%)", "Μέσος ρυθμός μεταβολής ΑΕΠ γεωργίας (%)")
        mysummary
    ...
    output$agri_real_gdp <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Μέσος ρυθμός μεταβολής πραγματικού ΑΕΠ γεωργίας παγκοσμίως",
            icon = icon("user"),
            color = "olive")
    })
    output$topcountry <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Χώρα με μεγαλύτερο ρυθμό μεταβολής πραγματικού ΑΕΠ γεωργίας (%)",
            icon = icon("globe"),
            color = "olive")
    })
    output$timeline<-renderPlot({ # Creating timeline for top 5 countries
        ggplot(mydata_top_five(), aes(x = year, y = agri_real_gdp, group = country, colour = country)) + 
            ...
            xlab("Έτος") + ylab("Ρυθμός μεταβολής ΑΕΠ γεωργίας (%)") + 
