...
header <- dashboardHeader(title = "Αγροτικοί Συνεταιρισμοί στην Ελλάδα", titleWidth=500) # Header of dashboard
...
server <- function(input, output, session) {
    ...
    output$regions<-renderPlot({ # Per region
        ggplot(coops_per_region, aes(x = factor(region))) + 
            ...
            xlab("Περιφέρεια") + ylab("Αριθμός συνεταιρισμών") + 
    ...
    output$prefectures<-renderPlot({ # Per prefecture
        ggplot(coops_per_pref, aes(x = factor(prefecture))) + 
            ...
            xlab("Περιφερειακή ενότητα") + ylab("Αριθμός συνεταιρισμών") + 
            theme(axis.text.x=element_text(angle=90, hjust=1)) + 
            geom_text(stat='count',aes(label=..count..),vjust=-1) + 
            theme(legend.title=element_blank()) + 
            theme(plot.title = element_text(family = "Trebuchet MS", color="#666666", face="bold", size=20)) +
