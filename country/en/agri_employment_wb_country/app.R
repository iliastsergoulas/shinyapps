...
# and creating plots and maps as WDI(country = "all", indicator = "SL.AGR.EMPL.ZS", extra = FALSE, cache = NULL)
...
mydata <- dbGetQuery(con, "SELECT * from agriculture.agri_employment_wb_country") # Get data
dbDisconnect(con)
dbUnloadDriver(drv)
...
header <- dashboardHeader(title = "Agricultural employment", titleWidth=500) # Header of dashboard
...
frow3 <- fluidRow(# Creating row of diagram and summary
    box(
        title = "5 countries with highest agricultural employment (%)",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
...
server <- function(input, output) {
    data_country <- reactive({ # Adding reactive data information
        ...
        colnames(data_country)<-c("Year", "Agricultural employment (%)")
        data_country
    })
    data_year <- reactive({ # Addding reactive data information
        ...
        colnames(data_year)<-c("Country", "Agricultural employment (%)")
        data_year
    })
    ...
    output$view <- renderGvis({ # Creating chart
        gvisColumnChart(data_country(), options=list(colors="['#336600']", vAxis="{title:'Agricultural employment (%)'}", 
                        hAxis="{title:'Year'}",backgroundColor="#d9ffb3", width=550, height=500, legend='none'))
    })
    output$map <- renderGvis({ # Creating map
        gvisGeoChart(data_year(), "Country", "Agricultural employment (%)", 
                     options=list(displayMode="regions", datamode='regions',width=550, height=500))
    })
    output$table <- renderDataTable({ # Creating data table
        colnames(mydata)<-c("Κωδικός", "Country", "Agricultural employment (%)", "Year")
        mydata[c("Country", "Agricultural employment (%)", "Year")]
    })
    output$summary <- renderDataTable({ # Creating summary by country
        ...
        colnames(mysummary) <- c("Country", "Minimum agricultural employment", "Maximum agricultural employment", "Mean agricultural employment")
        mysummary
    ...
    output$employment <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Mean agricultural employment globally",
            icon = icon("user"),
            color = "olive")
    })
    output$topcountry <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Country with highest agricultural employment",
            icon = icon("globe"),
            color = "olive")
    })
    output$timeline<-renderPlot({ # Creating timeline for top 5 countries
        ggplot(mydata_top_five(), aes(x = year, y = employment, group = country, colour = country)) + 
            ...
            xlab("Year") + ylab("Agricultural employment (%))") + 
