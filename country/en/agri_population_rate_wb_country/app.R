...
header <- dashboardHeader(title = "Rural population growth (%)", titleWidth=500) # Header of dashboard
...
frow3 <- fluidRow(# Creating row of diagram and summary
    box(
        title = "5 countries with highest rural population growth",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
...
server <- function(input, output) {
    data_country <- reactive({ # Adding reactive data information
        ...
        colnames(data_country)<-c("Year", "Rural population growth (%)")
        data_country
    })
    data_year <- reactive({ # Addding reactive data information
        ...
        colnames(data_year)<-c("Country", "Rural population growth (%)")
        data_year
    })
    ...
    output$map <- renderGvis({ # Creating map
        gvisGeoChart(data_year(), "Country", "Rural population growth (%)", 
                     options=list(displayMode="regions", datamode='regions',width=550, height=500))
    })
    output$table <- renderDataTable({ # Creating data table
        colnames(mydata)<-c("Code", "Country", "Rural population growth (%)", "Year")
        mydata[c("Country", "Rural population growth (%)", "Year")]
    })
    output$summary <- renderDataTable({ # Creating summary by country
        ...
        colnames(mysummary) <- c("Country", "Minimum rural population growth", "Maximum rural population growth", "Mean rural population growth")
        mysummary
    ...
    output$agri_population_rate <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Mean rural population growth globally",
            icon = icon("user"),
            color = "olive")
    })
    output$topcountry <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Country with highest rural population growth",
            icon = icon("globe"),
            color = "olive")
    })
