...
# and creating plots and maps as WDI(country = "all", indicator = "SL.AGR.0714.ZS", extra = FALSE, cache = NULL)
...
mydata <- dbGetQuery(con, "SELECT * from agriculture.agri_child_employment_wb_country") # Get data
dbDisconnect(con)
dbUnloadDriver(drv)
...
meanvalue<-mean((aggregate(mydata_filtered$child_employment, by=list(year=mydata_filtered$year), FUN=mean)$x)) # Mean value
topc<-mydata_filtered[which.max(mydata_filtered$child_employment),] # Top country
header <- dashboardHeader(title = "Child employment in agriculture", titleWidth=500) # Header of dashboard
sidebar <- dashboardSidebar(disable = TRUE)# Disabling sidebar of dashboard
frow1 <- fluidRow( # Creating row of valueboxes
    valueBoxOutput("childemployment", width=6),
...
frow3 <- fluidRow(# Creating row of diagram and summary
    box(
        title = "5 countries with highest child employment in agriculture (%)",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
...
server <- function(input, output) {
    data_country <- reactive({ # Adding reactive data information
        ...
        colnames(data_country)<-c("Year", "Child employment in agriculture (%)")
        data_country
    })
    data_year <- reactive({ # Adding reactive data information
        ...
        colnames(data_year)<-c("Country", "Child employment in agriculture (%)")
        data_year
    })
    ...
    output$view <- renderGvis({ # Creating chart
        gvisColumnChart(data_country(), options=list(colors="['#336600']", vAxis="{title:'Employment (%)'}", 
                        hAxis="{title:'Year'}",backgroundColor="#d9ffb3", width=550, height=500, legend='none'))
    })
    output$map <- renderGvis({ # Creating map
        gvisGeoChart(data_year(), "Country", "Child employment in agriculture (%)", 
                     options=list(displayMode="regions", datamode='regions',width=550, height=500))
    })
    output$table <- renderDataTable({ # Creating data table
        colnames(mydata)<-c("Κωδικός", "Country", "Child employment in agriculture (%)", "Year")
        mydata[c("Country", "Child employment in agriculture (%)", "Year")]
    })
    output$summary <- renderDataTable({ # Creating summary by country
        ...
        colnames(mysummary) <- c("Country", "Minimum child employment in agriculture", "Maximum child employment in agriculture", "Mean child employment in agriculture")
        mysummary
    }, options = list(lengthMenu = c(5, 25, 50), pageLength = 5))
    output$childemployment <- renderValueBox({ # Filling valuebox
        valueBox(
            paste0(specify_decimal(meanvalue, 2), " %"),
            "Mean child employment in agriculture globally",
            icon = icon("child"),
            color = "olive")
    })
    output$topcountry <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Country with highest child employment (%) in agriculture",
            icon = icon("globe"),
            color = "olive")
    })
    output$timeline<-renderPlot({ # Creating timeline for top 5 countries
        ggplot(mydata_top_five(), aes(x = year, y = child_employment, group = country, colour = country)) + 
            ...
            xlab("Year") + ylab("Child employment in agriculture (%)") + 
