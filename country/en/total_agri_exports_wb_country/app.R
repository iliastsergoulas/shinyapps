...
header <- dashboardHeader(title = "Exports of agricultural products (value $)", titleWidth=500) # Header of dashboard
...
frow3 <- fluidRow(# Creating row of diagram and summary
    box(
        title = "5 countries with highest value of agricultural products' exports",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
...
server <- function(input, output) {
    data_country <- reactive({ # Adding reactive data information
        ...
        colnames(data_country)<-c("Year", "Total exports ($)")
        data_country
    })
    data_year <- reactive({ # Adding reactive data information
        ...
        colnames(data_year)<-c("Country", "Total exports ($)")
        data_year
    })
    ...
    output$summary <- renderDataTable({ # Creating summary by country
        ...
        colnames(mysummary) <- c("Country", "Minimum exports value", "Maximum  exports value", "Mean  exports value")
        mysummary
    ...
    output$total_agri_exports <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Mean value of agricultural products' exports globally",
            icon = icon("dollar"),
            color = "olive")
    })
    output$topcountry <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Country with highest value of agricultural products' exports",
            icon = icon("globe"),
            color = "olive")
    })
