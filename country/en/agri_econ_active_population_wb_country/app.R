...
header <- dashboardHeader(title = "Active agricultural population", titleWidth=500) # Header of dashboard
...
frow3 <- fluidRow(# Creating row of diagram and summary
    box(
        title = "5 countries with largest active agricultural population",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
...
server <- function(input, output) {
    ...
    output$summary <- renderDataTable({ # Creating summary by country
        ...
        colnames(mysummary) <- c("Country", "Minimum active agricultural population", "Maximum active agricultural population", "Mean active agricultural population")
        mysummary
    ...
    output$agri_econ_population <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Mean active agricultural population globally",
            icon = icon("user"),
            color = "olive")
    })
    output$topcountry <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Country with largest active agricultural population",
            icon = icon("globe"),
            color = "olive")
    })
