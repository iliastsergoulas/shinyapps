# Data: Rural population (% of total population)
# This R script is created as a Shiny application processing raw data downloaded from World Bank through WDI package, 
# and creating plots and maps as WDI(country = "all", indicator = "SP.RUR.TOTL.ZS", extra = FALSE, cache = NULL)
...
mydata<-WDI(country = "all", indicator = "SP.RUR.TOTL.ZS", extra = FALSE, cache = NULL) # Downloading raw data from World Bank
credentials<-read.csv("/home/iliastsergoulas/dbcredentials.csv")
drv <- dbDriver("PostgreSQL") # loads the PostgreSQL driver
con <- dbConnect(drv, dbname = as.character(credentials$database), # creates a connection to the postgres database
                 host = as.character(credentials$host), port = as.character(credentials$port), 
                 user = as.character(credentials$user), password = as.character(credentials$password))
mydata <- dbGetQuery(con, "SELECT * from agriculture.agri_population_percentage_wb_country") # Get data
dbDisconnect(con)
dbUnloadDriver(drv)
mydata$year<-as.character(mydata$year)
names(mydata)[names(mydata)=="SP.RUR.TOTL.ZS"] <- "agri_population_percentage"
mydata<-mydata[which(!is.na(mydata$agri_population_percentage)),] # Filtering for NA values
...
meanvalue<-mean((aggregate(mydata_filtered$agri_population_percentage, by=list(year=mydata_filtered$year), FUN=mean)$x)) # Mean value
topc<-mydata_filtered[which.max(mydata_filtered$agri_population_percentage),] # Top country
header <- dashboardHeader(title = "Agricultural population (%)", titleWidth=500) # Header of dashboard
sidebar <- dashboardSidebar(disable = TRUE)# Disabling sidebar of dashboard
frow1 <- fluidRow( # Creating row of valueboxes
    valueBoxOutput("agri_population_percentage", width=6),
...
frow3 <- fluidRow(# Creating row of diagram and summary
    box(
        title = "5 countries with largest agricultural population",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
...
server <- function(input, output) {
    data_country <- reactive({ # Adding reactive data information
        data_country<-mydata[mydata$country==input$country, c("year", "agri_population_percentage")]
        data_country<-aggregate(data_country$agri_population_percentage, by=list(Year=data_country$year), FUN=sum)
        colnames(data_country)<-c("Year", "Agricultural population (%)")
        data_country
    })
    data_year <- reactive({ # Addding reactive data information
        data_year<-mydata_filtered[mydata_filtered$year==input$year,  c("country", "agri_population_percentage")]
        data_year<-aggregate(data_year$agri_population_percentage, by=list(Country=data_year$country), FUN=sum)
        colnames(data_year)<-c("Country", "Agricultural population (%)")
        data_year
    })
    mydata_top_five<-reactive({ # Subsetting data according to year interval and getting top five countries
        ...
        data_year_temp<-aggregate(mydata_top_five$agri_population_percentage, by=list(Country=mydata_top_five$country), FUN=mean)
    ...
    output$view <- renderGvis({ # Creating chart
        gvisColumnChart(data_country(), options=list(colors="['#336600']", vAxis="{title:'Agricultural population (%)'}", 
                        hAxis="{title:'Year'}",backgroundColor="#d9ffb3", width=550, height=500, legend='none'))
    })
    output$map <- renderGvis({ # Creating map
        gvisGeoChart(data_year(), "Country", "Agricultural population (%)", 
                     options=list(displayMode="regions", datamode='regions',width=550, height=500))
    })
    output$table <- renderDataTable({ # Creating data table
        colnames(mydata)<-c("Code", "Country", "Agricultural population (%)", "Year")
        mydata[c("Country", "Agricultural population (%)", "Year")]
    })
    output$summary <- renderDataTable({ # Creating summary by country
        mysummary <- data.frame(
            aggregate(agri_population_percentage~country, mydata_summary(), min),
            aggregate(agri_population_percentage~country, mydata_summary(), max),
            aggregate(agri_population_percentage~country, mydata_summary(), mean))
        mysummary <- mysummary[,c(1,2,4,6)]
        colnames(mysummary) <- c("Country", "Minimum agricultural population", "Maximum agricultural population", "Mean agricultural population")
        mysummary
    }, options = list(lengthMenu = c(5, 25, 50), pageLength = 5))
    output$agri_population_percentage <- renderValueBox({ # Filling valuebox
        valueBox(
            paste0(specify_decimal(meanvalue,2), " %"),
            "Mean agricultural population globally",
            icon = icon("user"),
            color = "olive")
    })
    output$topcountry <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Country with largest agricultural population",
            icon = icon("globe"),
            color = "olive")
    })
    output$timeline<-renderPlot({ # Creating timeline for top 5 countries
        ggplot(mydata_top_five(), aes(x = year, y = agri_population_percentage, group = country, colour = country)) + 
            geom_line() +
