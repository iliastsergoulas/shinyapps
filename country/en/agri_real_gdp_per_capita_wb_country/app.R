...
# and creating plots and maps as WDI(country = "all", indicator = "NV.AGR.PCAP.KD.ZG", extra = FALSE, cache = NULL)
...
mydata <- dbGetQuery(con, "SELECT * from agriculture.agri_real_gdp_per_capita_wb_country") # Get data
dbDisconnect(con)
dbUnloadDriver(drv)
...
meanvalue<-mean((aggregate(mydata_filtered$agri_real_gdp_per_capita, by=list(year=mydata_filtered$year), FUN=mean)$x)) # Mean value
topc<-mydata_filtered[which.max(mydata_filtered$agri_real_gdp_per_capita),] # Top country
header <- dashboardHeader(title = "Real agricultural GDP per capita growth rate (%)", titleWidth=500) # Header of dashboard
sidebar <- dashboardSidebar(disable = TRUE)# Disabling sidebar of dashboard
frow1 <- fluidRow( # Creating row of valueboxes
    valueBoxOutput("agri_real_gdp_per_capita", width=6),
...
frow3 <- fluidRow(# Creating row of diagram and summary
    box(
        title = "5 countries with highest real agricultural GDP per capita growth rate (%)",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
...
server <- function(input, output) {
    data_country <- reactive({ # Adding reactive data information
        ...
        colnames(data_country)<-c("Year", "Real agricultural GDP per capita growth rate (%)")
        data_country
    })
    data_year <- reactive({ # Adding reactive data information
        ...
        colnames(data_year)<-c("Country", "Real agricultural GDP per capita growth rate (%)")
        data_year
    })
    ...
    output$map <- renderGvis({ # Creating map
        gvisGeoChart(data_year(), "Country", "Real agricultural GDP per capita growth rate (%)", 
                     options=list(displayMode="regions", datamode='regions',width=550, height=500))
    })
    output$table <- renderDataTable({ # Creating data table
        colnames(mydata)<-c("Κωδικός", "Country", "Real agricultural GDP per capita growth rate (%)", "Year")
        mydata[c("Country", "Real agricultural GDP per capita growth rate (%)", "Year")]
    })
    output$summary <- renderDataTable({ # Creating summary by country
        ...
        colnames(mysummary) <- c("Country", "Minimum real agricultural GDP per capita growth rate", "Maximum real agricultural GDP per capita growth rate", "Mean real agricultural GDP per capita growth rate")
        mysummary
    ...
    output$agri_real_gdp_per_capita <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Mean real agricultural GDP per capita growth rate globally",
            icon = icon("user"),
            color = "olive")
    })
    output$topcountry <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Country with highest real agricultural GDP per capita growth rate",
            icon = icon("globe"),
            color = "olive")
    })
    output$timeline<-renderPlot({ # Creating timeline for top 5 countries
        ggplot(mydata_top_five(), aes(x = year, y = agri_real_gdp_per_capita, group = country, colour = country)) + 
            ...
            xlab("Year") + ylab("Real agricultural GDP per capita growth rate") + 
