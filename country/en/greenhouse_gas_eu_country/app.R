# Data: Greenhouse gas emissions from agriculture (source: EEA)
...
# and creating plots and maps as get_eurostat("env_air_gge", time_format = "raw")
...
header <- dashboardHeader(title = "Greenhouse gas production in EU (millions tonnes)", titleWidth=500) # Header of dashboard
...
frow3 <- fluidRow(# Creating row of diagram and summary
    box(
        title = "5 countries with highest greenhouse gas production",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
...
server <- function(input, output) {
    data_country <- reactive({ # Adding reactive data information
        ...
        colnames(data_country)<-c("Year", "Greenhouse gas production")
        data_country
    })
    data_year <- reactive({ # Adding reactive data information
        ...
        colnames(data_year)<-c("Country", "Greenhouse gas production")
        data_year
    })
    ...
    output$view <- renderGvis({ # Creating chart
        gvisColumnChart(data_country(), options=list(colors="['#336600']", vAxis="{title:'Greenhouse gas production (millions tonnes)'}", hAxis="{title:'Year'}",
                                                     backgroundColor="#d9ffb3", width=550, height=500, legend='none'))
    })
    output$map <- renderGvis({ # Creating map
        gvisGeoChart(data_year(), "Country", "Greenhouse gas production", options=list(region="150", displayMode="regions", 
                                                                                datamode='regions', width=550, height=500))
    })
    output$table <- renderDataTable({ # Creating data table
        colnames(mydata)<-c("Country", "Year", "Παραγωγή ρύπων")
        mydata
    })
    ...
    output$quantity <- renderValueBox({ # Filling valuebox
        valueBox(
            paste0(specify_decimal(meanvalue,2), " millions tonnes"),
            "Mean greenhouse gas production",
            icon = icon("user"),
            color = "olive")
    })
    output$topcountry <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Country with largest greenhouse gas production",
            icon = icon("globe"),
            color = "olive")
    })
    output$timeline<-renderPlot({ # Creating timeline for top 5 countries
        ggplot(mydata_top_five(), aes(x = year, y = quantity, group = country, colour = country)) + 
            ...
            xlab("Year") + ylab("Greenhouse gas production (millions tonnes)") + 
