...
server <- function(input, output) {
    data_country <- reactive({ # Add reactive data information
        ...
        colnames(data_country)<-c("Έτος", "Καλλιεργήσιμη γη")
        data_country
    })
    ...
    output$view <- renderGvis({ # Creating chart
        gvisColumnChart(data_country(), options=list(colors="['#336600']", vAxis="{title:'Καλλιεργήσιμη γη (εκτάρια)'}", hAxis="{title:'Έτος'}",
    ...
    output$map <- renderPlot({ # Creating map
        ...
        p <- p + guides(fill = guide_legend(title = "Καλλιεργήσιμη γη (εκτάρια)",
                                            title.position = "top", 
                                            title.hjust=0))
        p <- p + labs(title = "Μέγεθος ζωικού κεφαλαίου ανά περιφέρεια Ε.Ε.")
        print(p)
    })
    output$table <- renderDataTable({ # Creating data table
        colnames(mydata)<-c("Περιφέρεια", "Έτος", "Καλλιεργήσιμη γη")
        mydata
    })
}
shinyApp(ui, server)