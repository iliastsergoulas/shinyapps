...
server <- function(input, output) {
    data_country <- reactive({ # Add reactive data information
        ...
        colnames(data_country)<-c("Έτος", "Αριθμός γεωργικών εκμεταλλεύσεων")
        data_country
    })
    ...
    output$view <- renderGvis({ # Creating chart
        gvisColumnChart(data_country(), options=list(colors="['#336600']", vAxis="{title:'Αριθμός γεωργικών εκμεταλλεύσεων'}", hAxis="{title:'Έτος'}",
    ...
    output$table <- renderDataTable({ # Creating data table
        colnames(mydata)<-c("Περιφέρεια", "Έτος", "Αριθμός γεωργικών εκμεταλλεύσεων")
        mydata
    })
}
shinyApp(ui, server)