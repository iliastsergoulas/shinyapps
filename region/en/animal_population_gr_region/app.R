...
mydata <- dbGetQuery(con, "SELECT * from agriculture.animal_population_gr_region") # Get data
dbDisconnect(con)
dbUnloadDriver(drv)
...
header <- dashboardHeader(title = "Animal population (thousands heads) in Greek regions", titleWidth=600) # Header of dashboard
...
frow3 <- fluidRow(# Creating row of diagram and summary
    box(
        title = "5 περιφέρειες με υψηλότερο Animal population",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
...
server <- function(input, output) {
    ...
    output$number <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Mean animal population in Greek regions",
            icon = icon("user"),
            color = "olive")
    })
    output$topregion <- renderValueBox({ # Filling valuebox
        valueBox(
            topc$region,
            "Region with largest animal population in Greece",
            icon = icon("globe"),
            color = "olive")
    })
