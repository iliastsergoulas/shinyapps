...
mydata<-read.csv("C:\\Users\\Ilias Tsergoulas\\Dropbox\\Website\\shiny\\subsidies\\payments_paa_20142020\\payments.csv", sep=",", encoding="UTF-8", stringsAsFactors = FALSE)
...
server <- function(input, output) {
    ...
    output$view <- renderGvis({ # Creating chart
        gvisLineChart(data_fund(), options=list(colors="['#336600']", title="Απορρόφηση πόρων Προγράμματος Αγροτικής Ανάπτυξης 2014-2020", 
    ...
    output$viewyear<-renderPlot({ # Creating cross-check by years
        ggplot(data_year(), aes(x = month, y = total, fill=year)) + 
            ...
            ggtitle("Απορρόφηση πόρων Προγράμματος Αγροτικής Ανάπτυξης 2014-2020 και αντιπαραβολή ανά μήνα και έτος") + 
