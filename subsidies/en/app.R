...
header <- dashboardHeader(title = paste0("Course of subsidies (last update ",lastdate, ")"), 
                          titleWidth=1000) # Header of dashboard
sidebar <- dashboardSidebar(sidebarMenu(
    selectInput('fund', 'Fund', choices = unique(mydata$fund),selected='ΕΓΤΑΑ'),
    ...
    tags$footer(tags$p("This application is based on processed data from OPEKEPE's (Payment Agency) press releases.
                       Website agristats.eu is in no way responsible for the quality of the raw data. 
                       Its view remains as in that information is a public good and musdt be available as open processable data.")))
frow1 <- fluidRow( # Creating row of two diagrams
    tabsetPanel(
        tabPanel("Temporal depiction",dygraphOutput("view_total"),
                 dygraphOutput("view"),
                 dataTableOutput("summary"),width='98%'),
        tabPanel("Statistics per programme",print("These statistics refer to the selected period."),
        ...
        tabPanel("Statistics per measure",print("These statistics refer to the selected period."),
        ...
        tabPanel("Compare programs annually", plotOutput("category_per_year"), width='98%'),
        tabPanel("Compare measures annually", plotOutput("measure_per_year", height=600), width='98%'),
        tabPanel("Download data", downloadButton("downloadData"), width='98%'))
)
...
server <- function(input, output) {
    ...
    output$view_total <- renderDygraph({ # Creating chart
        dygraph(data_total()$total, main=paste0("Programme course: ", input$category))%>%
            dyAxis("y", label = "Payments", axisLabelFormatter=JS(valueFormatter))%>%
    ...
    output$summary <- renderDataTable({ # Creating summary by measure
        ...
        colnames(mysummary) <- c("Measure", "Minimum daily payments", "Maximum daily payments", "Mean daily payments")
        mysummary
    ...
    output$data_category <- renderDataTable({ # Creating summary by water_type
        ...
        names(data_category)<-c("Programme", "Payments")
    ...
    output$data_measure <- renderDataTable({ # Creating summary by water_type
        ...
        names(data_measure)<-c("Measure", "Payments","Contribution (%) to programme's total payments")
