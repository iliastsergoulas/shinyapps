...
mydata<-read.csv("C:\\Users\\Ilias Tsergoulas\\Dropbox\\Website\\shiny\\subsidies\\payments_direct_subsidies_measures\\payments.csv", sep=",", encoding="UTF-8", stringsAsFactors = FALSE)
mydata<-mydata[which(mydata$category=='Μέτρα Αγοράς'), ] # Filtering data
...
server <- function(input, output) {
    ...
    mydata_top_five<-reactive({ # Subsetting data according to year interval and getting top five measures
    ...
    output$view <- renderGvis({ # Creating chart
        gvisLineChart(data_measure(), options=list(colors="['#336600']", title="Μέτρα Αγοράς (ΕΓΤΕ) ανά Μέτρο", 
    ...
    output$timeline<-renderPlot({ # Creating timeline for top 5 countries
        ggplot(mydata_top_five(), aes(x = date, y = cumsum, group = measure, colour = measure)) + 
            ...
            xlab("Ημερομηνία") + ylab("Πληρωμές (Ευρώ)") + ggtitle("5 Μέτρα Αγοράς με μεγαλύτερη απορρόφηση πόρων") + 
