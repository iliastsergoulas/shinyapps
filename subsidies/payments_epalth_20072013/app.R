...
server <- function(input, output) {
    ...
    output$view <- renderGvis({ # Creating chart
        gvisLineChart(data_fund(), options=list(colors="['#336600']", title="Απορρόφηση πόρων Επιχειρησιακού Προγράμματος Αλιείας 2007-2013", 
    ...
    output$viewyear<-renderPlot({ # Creating cross-check by years
        ggplot(data_year(), aes(x = month, y = total, fill=year)) + 
            ...
            ggtitle("Απορρόφηση πόρων Επιχειρησιακού Προγράμματος Αλιείας 2007-2013 και αντιπαραβολή ανά μήνα και έτος") + 
