...
plants_1 <- plants[which(plants$type_en=='Manufacturer of PAP processing machines'),]
plants_2 <- plants[which(plants$type_en=='Manufacturer of essential oil distilleries'),]
plants_3 <- plants[which(plants$type_en=='Aromatic plants distillery'),]
...
header <- dashboardHeader(title = "Pharmaceutical and Aromatic Plants businesses", titleWidth=700) # Header of dashboard
    ...
    output$map_points <- renderLeaflet({
        leaflet() %>%
            ...
            addAwesomeMarkers(data = plants_1, icon = icon.fa,
                       popup = ~htmlEscape(business_n), group="Manufacturers of PAP processing machines") %>%
            addAwesomeMarkers(data = plants_2,icon = icon.glyphicon,
                              popup = ~htmlEscape(business_n), group="Manufacturers of essential oil distilleries") %>% 
            addAwesomeMarkers(data = plants_3,icon = icon.ion,
                              popup = ~htmlEscape(business_n), group="Aromatic plants distilleries") %>% 
            addLayersControl(
                overlayGroups = c("Manufacturers of PAP processing machines", 
                                  "Manufacturers of essential oil distilleries", 
                                  "Aromatic plants distilleries"),
