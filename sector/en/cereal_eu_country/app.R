...
mydata <- dbGetQuery(con, "SELECT * from agriculture.cereal_eu_country") # Get data
dbDisconnect(con)
dbUnloadDriver(drv)
...
header <- dashboardHeader(title = "Cereal production in EU countries", titleWidth=500) # Header of dashboard
...
frow3 <- fluidRow(# Creating row of diagram and summary
    box(
        title = "5 countries with largest cereal production",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
...
server <- function(input, output) {
    ...
    output$topcountry <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Country with largest cereal production in EU",
            icon = icon("globe"),
            color = "olive")
    })
