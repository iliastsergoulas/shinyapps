...
# and creating plots and maps as get_eurostat("apro_acs_a", time_format = "raw")
...
mydata <- dbGetQuery(con, "SELECT * from agriculture.fresh_vegetables_eu_country") # Get data
dbDisconnect(con)
dbUnloadDriver(drv)
...
header <- dashboardHeader(title = "Fresh vegetables production in EU countries", titleWidth=500) # Header of dashboard
...
frow3 <- fluidRow(# Creating row of diagram and summary
    box(
        title = "5 countries with largest fresh vegetables production in EU",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
...
server <- function(input, output) {
    ...
    output$view <- renderGvis({ # Creating chart
        gvisColumnChart(data_country(), options=list(colors="['#336600']", vAxis="{title:'Production (tonnes)'}", hAxis="{title:'Year'}",
                                                     backgroundColor="#d9ffb3", width=550, height=500, legend='none'))
    })
    output$map <- renderGvis({ # Creating map
        gvisGeoChart(data_year(), "Country", "Production", options=list(region="150", displayMode="regions", 
    ...
    output$quantity <- renderValueBox({ # Filling valuebox
        valueBox(
            paste0(specify_decimal(meanvalue,2), " tonnes"),
            "Mean fresh vegetables production in EU",
            icon = icon("user"),
            color = "olive")
    })
    output$topcountry <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Country with largest fresh vegetables production",
            icon = icon("globe"),
            color = "olive")
    })
