...
mydata <- dbGetQuery(con, "SELECT * from agriculture.bio_production_cereal_eu_country") # Get data
dbDisconnect(con)
dbUnloadDriver(drv)
...
header <- dashboardHeader(title = "Organic cereal production in EU countries", titleWidth=550) # Header of dashboard
...
frow3 <- fluidRow(# Creating row of diagram and summary
    box(
        title = "5 countries with largest organic cereal production",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
