# Data: Production and utilization of milk on the farm - annual data
...
mydata<-get_eurostat("apro_mk_farm", time_format = "raw") # Downloading raw data from Eurostat
mydata$geo<-as.character(mydata$geo)
mydata<-mydata[which(mydata$dairyprod=='D1100A' & mydata$milkitem=='PRO'), ] # Filtering data
...
header <- dashboardHeader(title = "Milk production in EU countries", titleWidth=500) # Header of dashboard
...
frow3 <- fluidRow(# Creating row of diagram and summary
    box(
        title = "5 countries with largest milk production in EU",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
...
server <- function(input, output) {
    ...
    output$view <- renderGvis({ # Creating chart
        gvisColumnChart(data_country(), options=list(colors="['#336600']", vAxis="{title:'Production (tonnes)'}", 
