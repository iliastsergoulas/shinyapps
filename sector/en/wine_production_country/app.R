...
# and creating plots and maps as get_eurostat("apro_cpb_wine", time_format = "raw")
...
mydata <- dbGetQuery(con, "SELECT * from agriculture.wine_production_eu_country") # Get data
dbDisconnect(con)
dbUnloadDriver(drv)
...
header <- dashboardHeader(title = "Wine production in EU countries", titleWidth=500) # Header of dashboard
...
server <- function(input, output) {
    ...
    output$view <- renderGvis({ # Creating chart
        gvisColumnChart(data_country(), options=list(colors="['#336600']", vAxis="{title:'Production (thousands hL)'}", 
    ...
    output$timeline<-renderPlot({ # Creating timeline for top 5 countries
        ggplot(mydata_top_five(), aes(x = year, y = quantity, group = country, colour = country)) + 
            ...
            xlab("Year") + ylab("Production (thousands hL)") + 
