...
header <- dashboardHeader(title = "Aquaculture production in EU countries (1984-2007)", titleWidth=500) # Header of dashboard
...
frow3 <- fluidRow(# Creating row of diagram and summary
    box(
        title = "5 top countries with largest aquaculture production",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
...
server <- function(input, output) {
    ...
    output$topcountry <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Country with largest aquaculture production",
            icon = icon("globe"),
            color = "olive")
    })
