# Data: Production from aquaculture excluding hatcheries and nurseries (1984-2007) 
...
# and creating plots and maps as get_eurostat("fish_aq_q", time_format = "raw")
...
mydata <- dbGetQuery(con, "SELECT * from agriculture.aquaculture_production_eu_country_8407") # Get data
dbDisconnect(con)
dbUnloadDriver(drv)
...
header <- dashboardHeader(title = "Παραγωγή από υδατοκαλλιέργειες (1984-2007)", titleWidth=500) # Header of dashboard
...
frow3 <- fluidRow(# Creating row of diagram and summary
    box(
        title = "5 χώρες με μεγαλύτερη παραγωγή από υδατοκαλλιέργειες",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
...
server <- function(input, output) {
    ...
    output$view <- renderGvis({ # Creating chart
        gvisColumnChart(data_country(), options=list(colors="['#336600']", vAxis="{title:'Παραγωγή (χιλ. τόνοι)'}", 
                        hAxis="{title:'Έτος'}", backgroundColor="#d9ffb3", width=550, height=500, legend='none'))
    })
    output$map <- renderGvis({ # Creating map
        gvisGeoChart(data_year(), "Χώρα", "Παραγωγή", options=list(region="150", 
    ...
    output$topcountry <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Χώρα με μεγαλύτερη παραγωγή από υδατοκαλλιέργειες",
            icon = icon("globe"),
            color = "olive")
    })
