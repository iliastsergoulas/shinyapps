...
dbDisconnect(con)
dbUnloadDriver(drv)
names(mydata)<-c("year", "water_type","Επιστημονικό προσωπικό", "Λοιπό προσωπικό","Συμβασιούχοι")
...
header <- dashboardHeader(title = "Απασχόληση σε υδατοκαλλιέργειες", titleWidth=500) # Header of dashboard
...
frow2 <- fluidRow( # Creating row of two diagrams
    box(
        ...
        mainPanel(
            ...
            print("Πηγή: ΕΛΣΤΑΤ"),
            selectInput('water_type', 'Τύπος υδάτων', choices = unique(mydata$water_type)), width='98%')),
    box(
        title = "Ανά τύπο απασχολούμενου",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
...
frow3 <- fluidRow(# Creating row of diagram and summary
    box(
        title = "Χρονική εξέλιξη απασχόλησης υδατοκαλλιεργειών",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
    ...
    box(
        title = "Σύνοψη δεδομένων ανά τύπο υδάτων",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
...
server <- function(input, output) {
    data_water_type <- reactive({ # Adding reactive data information
        ...
        colnames(data_water_type)<-c("Έτος", "Απασχόληση")
        data_water_type
    })
    data_employment_type <- reactive({ # Addding reactive data information
        ...
        colnames(data_employment_type)<-c("Είδος απασχόλησης", "Απασχόληση")
    ...
    output$view_water <- renderGvis({ # Creating chart
        gvisColumnChart(data_water_type(), options=list(colors="['#336600']", vAxis="{title:'Απασχόληση'}", 
                        hAxis="{title:'Έτος'}",backgroundColor="#d9ffb3", width=550, height=500, legend='none'))
    })
    output$view_employment <- renderGvis({ # Creating map
        gvisColumnChart(data_employment_type(), options=list(colors="['#336600']", vAxis="{title:'Απασχόληση'}", 
                        hAxis="{title:'Είδος απασχόλησης'}",backgroundColor="#d9ffb3", width=550, height=500, legend='none'))
    })
    output$summary <- renderDataTable({ # Creating summary by water_type
        ...
        colnames(mysummary) <- c("Τύπος υδάτων", "Ελάχιστη απασχόληση", "Μέγιστη απασχόληση", "Μέση απασχόληση")
        mysummary
    ...
    output$topwater_type <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Έτος με μεγαλύτερη απασχόληση",
            icon = icon("globe"),
            color = "olive")
    })
    output$timeline<-renderGvis({ # Creating timeline for
        gvisLineChart(mydata_timeline(), options=list(colors="['#336600']", vAxis="{title:'Απασχόληση'}", 
                    hAxis="{title:'Έτος'}",backgroundColor="#d9ffb3", width=550, height=500, legend='none'))
    })
