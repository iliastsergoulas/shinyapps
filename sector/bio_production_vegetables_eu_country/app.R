...
server <- function(input, output) {
    ...
    output$view <- renderGvis({ # Creating chart
        gvisColumnChart(data_country(), options=list(colors="['#336600']", title="Παραγωγή βιολογικών νωπών λαχανικών στις χώρες της Ε.Ε.", 
    ...
    output$timeline<-renderPlot({ # Creating timeline for top 5 countries
        ggplot(mydata_top_five(), aes(x = year, y = quantity, group = country, colour = country)) + 
            ...
            xlab("Έτος") + ylab("Παραγωγή (τόνοι)") + ggtitle("5 κορυφαίες χώρες στην παραγωγή βιολογικών νωπών λαχανικών") + 
