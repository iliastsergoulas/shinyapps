...
# and creating plots and maps as get_eurostat("org_croppro", time_format = "raw")
...
mydata <- dbGetQuery(con, "SELECT * from agriculture.bio_production_vegetables_eu_country") # Get data
dbDisconnect(con)
dbUnloadDriver(drv)
...
header <- dashboardHeader(title = "Παραγωγή βιολογικών λαχανικών στις χώρες της Ε.Ε.", titleWidth=550) # Header of dashboard
...
frow3 <- fluidRow(# Creating row of diagram and summary
    box(
        title = "5 χώρες με τη μεγαλύτερη παραγωγή βιολογικών λαχανικών",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
