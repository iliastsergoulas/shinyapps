...
server <- function(input, output) {
    ...
    output$view <- renderGvis({ # Creating chart
        gvisColumnChart(data_country(), options=list(colors="['#336600']", title="Παραγωγή γάλακτος στις χώρες της Ε.Ε.", 
    ...
    output$timeline<-renderPlot({ # Creating timeline for top 5 countries
        ggplot(mydata_top_five(), aes(x = year, y = quantity, group = country, colour = country)) + 
            ...
            xlab("Έτος") + ylab("Παραγωγή (τόνοι)") + ggtitle("5 χώρες με τη μεγαλύτερη παραγωγή γάλακτος") + 
