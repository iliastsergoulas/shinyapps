...
names(mydata)<-c("year", "region", "Ίπποι αρσενικοί", "Ίπποι θηλυκοί", "Όνοι αρσενικοί", "Όνοι θηλυκοί")
...
header <- dashboardHeader(title = "Ιπποειδή στην Ελλάδα", titleWidth=500) # Header of dashboard
...
frow2 <- fluidRow( # Creating row of two diagrams
    box(
        title = "Πορεία πληθυσμού ιπποειδών ανά κατηγορία",
        status="success",
        collapsible = TRUE,
        ...
        mainPanel(
            ...
            print("Πηγή: ΕΛΣΤΑΤ"),
            selectInput('horses_category', 'Κατηγορία', choices = unique(mydata_processed$variable)), width='98%')),
    box(
        title = "Πορεία πληθυσμού ιπποειδών ανά Περιφέρεια",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
...
frow3 <- fluidRow( # Creating row of two diagrams
    box(
        title = "Σύνοψη δεδομένων ανά είδος ίππου",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
...
server <- function(input, output) {
    ...
    output$mean_horses_population <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Μέσος εθνικός πληθυσμός ιπποειδών ετησίως",
            icon = icon("map"),
            color = "olive")
    })
    output$topyear <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Έτος με μέγιστο συνολικό εθνικό πληθυσμό ιπποειδών",
            icon = icon("globe"),
            color = "olive")
    })
    ...
    output$summary <- renderDataTable({ # Creating summary by water_type
        ...
        colnames(mysummary) <- c("Έιδος ίππου", "Ελάχιστος πληθυσμός", "Μέγιστος πληθυσμός", "Μέσος πληθυσμός")
        mysummary
