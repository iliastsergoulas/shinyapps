...
header <- dashboardHeader(title = "Κέντρα μελισσοκομίας", titleWidth=700) # Header of dashboard
sidebar <- dashboardSidebar(disable = TRUE)# Disabling sidebar of dashboard
frow1 <- fluidRow( # Creating row of valueboxes
    leafletOutput("map_points")
)
frow2 <- fluidRow(# Creating row of diagram and summary
    title = "Στοιχεία κέντρων μελισσοκομίας",
    status="success",
    collapsible = TRUE,
...
server <- function(input, output, session) {
    ...
    output$list <- renderDataTable({ # Creating summary by country
        ...
        names(plants_list)<-c("Ονομασία", "Διεύθυνση", "Τηλέφωνο", "E-mail", "Φαξ")
