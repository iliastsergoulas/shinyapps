# Data: Beef population based on data from Hellenic Statistic Service
# This R script is created as a Shiny application to process raw data 
...
mydata <- dbGetQuery(con, "SELECT * from agriculture.beef_classes_elstat") # Get data
...
mean_beef_population<-mean(total_per_year$x) # Mean value
...
header <- dashboardHeader(title = "Εκμεταλλεύσεις βοοειδών κατά τάξη μεγέθους και Περιφέρεια στην Ελλάδα", titleWidth=750) # Header of dashboard
sidebar <- dashboardSidebar(disable = TRUE)# Disabling sidebar of dashboard
frow1 <- fluidRow( # Creating row of valueboxes
    valueBoxOutput("mean_beef_population", width=6),
    valueBoxOutput("topyear", width=6)
)
frow2 <- fluidRow( # Creating row of two diagrams
    box(
        title = "Πορεία αριθμού εκμεταλλεύσεων βοοειδών ανά τάξη μεγέθους",
        status="success",
        collapsible = TRUE,
        ...
        mainPanel(
            ...
            print("Πηγή: ΕΛΣΤΑΤ"),
            selectInput('beef_category', 'Κατηγορία', choices = unique(mydata_processed$variable)), width='98%')),
    box(
        title = "Πορεία εκμεταλλεύσεων βοοειδών κατά τάξη μεγέθους ανά Περιφέρεια",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
...
frow3 <- fluidRow( # Creating row of two diagrams
    box(
        title = "Χρονική ανάλυση διάρθρωσης εκμεταλλεύσεων βοοειδών κατά τάξη μεγέθους",
        status="success",
        collapsible = TRUE,
        theme = shinytheme("spacelab"), 
        mainPanel(
...
server <- function(input, output) {
    ...
    mydata_processed_timeline_category<-reactive({ # Filtering data by chosen wine category
        mydata_processed_timeline_category<-mydata_processed[which(mydata_processed$variable==input$beef_category),]
    })
    ...
    output$mean_beef_population <- renderValueBox({ # Filling valuebox
        valueBox(
            paste0(specify_decimal(mean_beef_population,2)),
            "Μέσος αριθμός εκμεταλλεύσεων βοοειδών ετησίως",
            icon = icon("map"),
            color = "olive")
    })
    output$topyear <- renderValueBox({ # Filling valuebox
        valueBox(
            ...
            "Έτος με μέγιστο αριθμό εκμεταλλεύσεων βοοειδών",
            icon = icon("globe"),
            color = "olive")
    })
